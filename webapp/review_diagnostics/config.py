...
# --- Product limits ---
# Restored to 200 after the PyTorch/BERTopic/UMAP/HDBSCAN stack (which
# pushed peak memory to the free-tier host's 512MB ceiling even at 120
# reviews) was replaced with fastembed (ONNX, no torch) + scikit-learn's
# built-in HDBSCAN (no numba) + PCA -- measured peak memory for a
# 120-review run dropped from ~511MB to ~407-482MB with that swap, leaving
# real headroom back up toward the original 200-review design target.
